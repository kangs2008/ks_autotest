...
from Common.handle_excel3 import excel_to_case, load_excel, excel_to_save, Handle_excel
...
path = r'D:\desk20201127\ks_web_allure\Datas\test_apidata.xlsx'
num = '1'
tmp_excel_path = r'D:\desk20201127\ks_web_allure\Report'


apidata = excel_to_case(path, 't_接', 't_', ['config'])
logger.info(apidata)
class TestAPI():
    def setup(self):
        self.proxies = {
    ...
    def test_all_api(self, data):
        ...
        exec_c, col_pos_c = Handle_excel(list(data.values())[2]).get_column_values_by_title(sheet, 'return_code')
        exec_v, col_pos_v = Handle_excel(list(data.values())[2]).get_column_values_by_title(sheet, 'return_values')
        ...
        for va in list(data.values())[3]:
            ...
            if va['method'].strip().lower().endswith('_api'):
                func(va['input'], data=va['request_data'], proxies=None)
            else:
                func(va['input'], data=va['request_data'])
    ...
    def load_excel_setup(self, sheet_name, file_name, file_path):
        ...
        write_file_path = os.path.join(tmp_excel_path, write_file_name)
