...
from Common.handle_excel4 import load_excel, excel_to_save, Handle_excel, write_to_excel
...
class TestAPI():
    ...
    def test_all_api(self, _data):
        ...
        allure.dynamic.description(f'FILE SHEET： {list(_data.values())[0]}  \n\nFILE INSERT SHEET： {list(_data.values())[1]}  \n\nFILE NAME： {list(_data.values())[2]}  \n\nFILE PATH： {list(_data.values())[3]}')
        logger.info(f'API_interface_test')
        logger.info(f'FILE SHEET： {list(_data.values())[0]}  FILE INSERT SHEET： {list(_data.values())[1]}  FILE NAME： {list(_data.values())[2]}  FILE PATH： {list(_data.values())[3]}')
