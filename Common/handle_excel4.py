...
class Handle_excel():
    ...
    def get_exec_dictList_from_sheet_re2(self, sheet_obj, config_list, insert_sheets, exec_value='exec', exec_type='y'):
        """
            ...
            for row in range(2, row_num + 1):
                ...
                if exec_column_values[row-2].lower() == exec_type:
                    ...
                    for column in range(1, column_num + 1):
                        ...
                        if str(value).strip().startswith('for_'):
            ...
            if isinstance(sheet_name_list, list):
                if len(sheet_name_list) != 0 and sheet_name_list[0] != '':
                    ...
                    for i, sheet_one in enumerate(sheet_name_list):
                        if len(name_rule_list) != 0:
                            if sheet_one in name_rule_list:
                                ...
                                insert_sheet, one_insert_sheet = __get_insert_sheets_re(rule_sheets, sheet_name_list, insert_rule='for_')
                                excel_kv_values = Handle_excel(file_name).get_exec_dictList_from_sheet_re2(sheet_obj, config_list, insert_sheet, exec_value, exec_type)
                    ...
                    for i, sheet_one in enumerate(name_rule_list):
                        ...
                        sheet_obj = Handle_excel(file_name).get_sheet_by_name(sheet_one)
                        rule_sheets = Handle_excel(file_name).get_sheets_by_rule('for_' + sheet_one)
                        insert_sheet, one_insert_sheet = __get_insert_sheets_re(rule_sheets, sheet_name_list,
                                                                                insert_rule='for_')
                ...
                if sheet_name_list in name_rule_list:
                    ...
                    insert_sheet, one_insert_sheet = __get_insert_sheets_re(rule_sheets, sheet_name_list,
                                                                            insert_rule='for_')
                    excel_kv_values = Handle_excel(file_name).get_exec_dictList_from_sheet_re2(sheet_obj, config_list,
                                                                                               insert_sheet, exec_value,
                                                                                               exec_type)
                    case_kv[f'sheet_name'] = sheet_one
                    case_kv[f'insert_sheet'] = one_insert_sheet
                    case_kv[f'file_name'] = name
                ...
                for i, sheet_one in enumerate(sheet_name_list):
                    if len(name_rule_list) != 0:
                        if sheet_one in name_rule_list:
                            ...
                            sheet_obj = Handle_excel(file_name).get_sheet_by_name(sheet_one)
                            rule_sheets = Handle_excel(file_name).get_sheets_by_rule('for_' + sheet_one)
                            insert_sheet, one_insert_sheet = __get_insert_sheets_re(rule_sheets, sheet_one,
                                                                                    insert_rule='for_')
                            excel_kv_values = Handle_excel(file_name).get_exec_dictList_from_sheet_re2(sheet_obj,
                                                                                                       config_list,
                                                                                                       insert_sheet,
